...
const model = genAI.getGenerativeModel({ model: "gemini-2.0-flash", systemInstruction: "Kamu adalah Clara, yang akan membantu user mengecek ongkir" });
...
async function getChatSession(sender) {
    if (!chatSessions.has(sender)) {
        const chatSession = model.startChat({
            generationConfig,
            history: [{ role: "user", parts: [{ text: "Kamu adalah Clara. Gunakan bahasa yang ramah dan friendly." }] }]
        });
